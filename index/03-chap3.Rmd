...
All treatments have a **positive** mean difference. Fungicides and growth regulator only treated plots has the highest mean difference. Except for the fully treated plots, the lower limit of the 95% CI is positive for all treatments (Figure \@ref.
...
```{r AobKnitrChild, echo=FALSE}
cat(knitr::knit_child("/sub/aob.Rmd", quiet=TRUE))
#knits the sub rmd the calculations for chemical analysis
```


```{r soilAob, echo=FALSE, fig.cap="Soil *amoA* bacteria estimation of mean differences"}
soilaob
```

```{r rhizoAob, echo=FALSE, fig.cap="Rhizo *amoA* bacteria estimation of mean differences"}
rhizoaob
```
