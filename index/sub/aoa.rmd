...
<!-- Data load -->
```{r aoa qPCR load, echo=FALSE, warning=FALSE, message = FALSE}
# Loading bay
qpcr <- 
...
<!-- not doing a recalibration --> 
```{r recalibration aoa, eval=FALSE, echo=FALSE, message=FALSE}
...
#Normalize quantity values
  ...
  # Index created: averaged quantity value (from technical replicates) for each replicate
   ...
   calc <- joined %>%
     mutate(qty_ng=avg.qty*400)%>% 
     mutate(qty_ng=qty_ng/8)%>% 
     mutate(qty_ng=qty_ng/dna_conc)
